...
@WebServlet("/admin/archive-notice")
public class ArchiveNoticeServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            boolean archived = noticeDAO.archiveNotice(noticeId);

            if (archived && userId != null) {
                Notice notice = noticeDAO.findById(noticeId);
                String title = notice != null ? notice.getTitle() : "Notice ID " + noticeId;
                auditLogDAO.insertLog(userId, "ARCHIVE_NOTICE", "Archived notice titled: " + title);
            }
