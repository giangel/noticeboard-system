...
@WebServlet("/admin/publish-notice")
public class PublishNoticeServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            boolean published = noticeDAO.publishNotice(noticeId);

            if (published && userId != null) {
                Notice notice = noticeDAO.findById(noticeId);
                String title = notice != null ? notice.getTitle() : "Notice ID " + noticeId;
                auditLogDAO.insertLog(userId, "PUBLISH_NOTICE", "Published notice titled: " + title);
            }
